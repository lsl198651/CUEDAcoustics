...
def train_and_validate_model(
    ...
    valid_dataset = RecordingDataset(
        data_folder, val_files, val_labels, val_timings, sequence_length=None, cache=shared_feature_cache
    )

    val_features, val_labels, val_lengths = batch_whole_dataset(valid_dataset)

    model = train_model(
        model_folder, fold, train_dataset, valid_dataset, use_gpu=gpu, verbose=verbose, quick=quick
    )
...
def predict_files(model, data_folder, files, labels, timings, cache, gpu):
    dataset = RecordingDataset(data_folder, files, labels, timings, sequence_length=None, cache=cache)
...
def train_model(model_folder, fold, train_dataset, valid_dataset, use_gpu, verbose, quick):
    ...
    optimiser = torch.optim.Adam(model.parameters(), lr=TRAINING_LR)
    ...
    #    Re-load best model
    model = load_single_network_fold(model_folder, fold)
    if use_gpu:
        model = model.cuda()
    return model
