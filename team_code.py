...
def load_challenge_model(model_folder, verbose):
    ...
    return [
        {
            "network": neural_networks.load_single_network_fold(model_folder, fold),
            "tree_murmur": decision_tree.load_catboost_model(model_folder, fold, "murmur_label") if USE_MURMUR_DECISION_TREE else None,
...
def run_challenge_model(model, data, recordings, verbose):
    ...
    for recording, name in zip(recordings, recording_name):
        ...
        fold_predictions = []
        for model_fold in model:
            results = neural_networks.predict_single_model(model_fold["network"], features, lengths, gpu=True)
...
def train_challenge_model_full(data_folder, model_folder, verbose, load_old_file=False, gpu=True, quick=False):
    ...
    # TODO: don't use "unknown" recordings to train RNN
    recording_rows = []
    ...
    if not load_old_file:
        ...
        for fold in fold_names:
            ...
            model, val_fold_results = neural_networks.train_and_validate_model(
...
def create_folds(patient_df: pd.DataFrame):
    # deterministic?
    skf = sklearn.model_selection.StratifiedKFold(n_splits=5, shuffle=True, random_state=1)
